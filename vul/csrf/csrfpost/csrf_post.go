...
// CsrfPostHandler 显示用户信息页面
func CsrfPostHandler(renderer templates.Renderer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// 检查用户是否登录
		loggedIn, username := utils.CheckCSRFLogin(r)
		if !loggedIn {
			http.Redirect(w, r, "/vul/csrf/login?module=post&redirect=/vul/csrf/csrfpost/csrf_post", http.StatusFound)
			return
		}
		...
		if r.URL.Query().Get("logout") == "1" {
			...
			http.Redirect(w, r, "/vul/csrf/login?module=post", http.StatusFound)
			return
		}
		...
		<div id="per_info">
		   ...
		   <a class="edit" href="/vul/csrf/csrfpost/csrf_post_edit">修改个人信息</a>
		</div>
		`
