...
// CsrfPostEditHandler 通过POST请求修改用户信息 - 故意存在CSRF漏洞
func CsrfPostEditHandler(renderer templates.Renderer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// 检查用户是否登录
		loggedIn, username := utils.CheckCSRFLogin(r)
		if !loggedIn {
			http.Redirect(w, r, "/vul/csrf/login?module=post&redirect=/vul/csrf/csrfpost/csrf_post_edit", http.StatusFound)
			return
		}
		...
		if r.Method == http.MethodPost {
			...
			if sex != "" && phonenum != "" && address != "" && email != "" {
				...
				if err != nil {
					message = "修改失败，请重试"
					log.Printf("CSRF POST修改失败: %v", err)
				} else {
					// 修改成功，重定向到会员中心
		...
		<div id="per_info">
		   <form method="post">
		   <h1 class="per_title">hello,` + username + `,欢迎来到个人会员中心 | <a style="color:blue;" href="/vul/csrf/csrfpost/csrf_post?logout=1">退出登录</a></h1>
		...
		renderer.RenderPage(w, "csrf/csrfpost/csrf_post_edit.html", data)
	}
}
